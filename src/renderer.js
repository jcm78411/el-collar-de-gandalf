...
document.getElementById("startBtn").addEventListener("click", () => {
  ...
  if (isNaN(n) || n <= 0) {
    // Validar que el número de perlas sea un número válido y positivo
    Swal.fire({
  ...
  perlas[perlas.length - 1] = "blanca"; // La última perla es 'blanca'
...
function ciclo() {
  // Función que maneja el ciclo de iteraciones
  ...
  // Verificar si se transforma la última perla
  if (perlas[0] === "blanca" && perlas[perlas.length - 1] === "negra") {
    perlas[perlas.length - 1] = "blanca";
    activas++;
    ultimaTransformada = perlas.length - 1;
  }
  ...
  if (negrasRestantes === 0) {
    ...
    Swal.fire({
      ...
      // text: `Total de iteraciones: ${iteraciones.length}`,
...
function mostrarCollar() {
  // Función para mostrar el collar circular con las perlas
  ...
  const radius = Math.max(
    minRadius,
    (perlaSize + padding) / (2 * Math.sin(angle / 2))
  );
  ...
  perlas.forEach((color, i) => {
    ...
    if (i === ultimaTransformada) {
      // Si es la última perla transformada, añadir una clase especial
...
function generarEncabezado(n) {
  // Función para generar el encabezado de la tabla de iteraciones
  ...
  for (let i = 1; i <= n; i++) {
    // Iterar desde 1 hasta n para crear las columnas de perlas
